...
stuff translate[128] = {/* \00x */ none,   none,   none, none,	none,	none,  none, none,
...
main(int argc, char *argv[])
{
	...
	if (errno != 0) {
	    fprintf(stderr, "ERROR: %s: file: %s line: %d dungeon: %u argv[2]: %s cannot be converted into an int: %s\n", __func__,
		    __FILE__, __LINE__, dnum, argv[2], strerror(errno));
	    exit(1);
	}
	if (rogpid < 0) {
	...
	i = sscanf(argv[3], "%d,%d,%d,%d,%d,%d,%d,%u,%ld,%ld,%d", &creative_int, &noterm_int, &startecho_int, &nohalf_int,
		   &emacs_int, &terse_int, &transparent_int, &dnum, &goodgame, &usleep_usec, &quiet_int);
	if (i != 11) {
	    fprintf(stderr,
		    "ERROR: %s: file: %s line: %d dungeon: %u argv[3]: %s "
    ...
    if (replaying) {
	snprintf(msg, SM_BUF, " Replaying log file %s, version %s.", logfilename, versionstr);
    } else {
	snprintf(msg, SM_BUF, " %s: version %s, genotype %d.", roguename, versionstr, geneid);
    }
	...
	if ((transparent && !singlestep) || (!emacs && charsavail()) || !strategize()) {
	    ...
	    case '<':
		if (atrow == stairrow && atcol == staircol && have(amulet) != NONE) {
		    command(T_OTHER, "<");
		}
	    ...
	    case 'G':
		mvprintw(0, 0, "%d: Sr %d Dr %d Re %d Ar %d Ex %d Rn %d Wk %d Fd %d, %d/%d", geneid, k_srch, k_door, k_rest,
			 k_arch, k_exper, k_run, k_wake, k_food, genebest, geneavg);
		clrtoeol();
		at(row, col);
		if (!quiet) {
		    refresh();
	    ...
	    case '~':
		if (replaying) {
		    saynow("Replaying log file %s, version %s.", logfilename, versionstr);
		} else {
		    saynow(" %s: version %s, genotype %d.", roguename, versionstr, geneid);
		}
	    ...
	    case '[':
		at(0, 0);
		printw("%s = %d, %s = %d, %s = %d, %s = %d.", "hitstokill", hitstokill, "goodweapon", goodweapon, "usingarrow",
		       usingarrow, "goodarrow", goodarrow);
		clrtoeol();
		at(row, col);
		if (!quiet) {
		    refresh();
	    ...
	    case 'E':
		dwait(D_ERROR, __func__, "Testing the ERROR trap");
		break;

	    case 'F':
		dwait(D_FATAL, __func__, "Testing the FATAL trap");
		break;
	    ...
	    case ROGQUIT:
		dwait(D_ERROR, __func__, "Strategize failed: gave up");
...
endlesson(void)
{
    ...
    if (geneid > 0 &&
	(stlmatch(termination, "perditus") || stlmatch(termination, "victorius") || stlmatch(termination, "callidus"))) {
