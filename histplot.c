...
main(int argc, char *argv[])
{
	...
	while (*++(*argv)) {
	    ...
	    case 'l':
		dolev++;
		break; /* Plot level instead of score */
	    case 'a':
		minscore = atoi(*argv + 1);
		SKIPARG;
		break;
	    default:
...
getscore(int *score, char *killer, int *level)
{
	...
	dd = yy = *score = 0;
	sscanf(line, "%s %d, %d %10s%d%c%17s", mmstr, &dd, &yy, player, score, &creativity, killer);
	...
	if (yy > 0 &&
	    ...
	    (creativity != '*' || creative) &&
#endif
	    !stlmatch("saved", killer) && (*score > 2000 || !stlmatch("user", killer)))
	    return (1);
    }
