...
int havearmor(int k, int print, int rustproof) {
    ...
    if (print) {
        ...
        for (i = 0; i < n; i++)
            mvprintw(i + 3, 8, "%2d: %3d %s", i + 1, armval[i],
                     itemstr(armind[i]));
    }
...
int armorclass(int i) {
    int class = 0;
    if (inven[i].type != armor) return (1000);
...
int haveweapon(int k, int print) {
    ...
    if (print) {
        ...
        for (i = 0; i < n; i++)
            mvprintw(i + 3, 8, "%2d: %5d %s", i + 1, weapval[i],
                     itemstr(weapind[i]));
    }
...
int weaponclass(int i) {
    ...
    if (inven[i].phit != UNKNOWN) {
        ...
        if (inven[i].pdam != UNKNOWN) damplus = inven[i].pdam;
    }
...
int havering(int k, int print) {
    ...
    if (print) {
        ...
        for (i = 0; i < n; i++)
            mvprintw(i + 3, 8, "%2d: %5d  %s", i + 1, ringval[i],
                     itemstr(ringind[i]));
    }
...
int ringclass(int i) {
    ...
    /* A ring of sustain strength */
        ...
        if (!itemis(i, INUSE) && wearing("sustain strength") != NONE)
            ...
            class =
                (havefood(3) ? 1000 : 0) +
                (Level > 12 ? 150
                            : Str > 2000 ? 700 : Str > 1600 ? Str - 1200 : 100);
    }
    ...
    /* A ring of see invisible */
        ...
        if (!itemis(i, INUSE) && wearing("see invisible") != NONE) class = 0;
         ...
         * turns, just in case the beast comes back to haunt us.
         * MLM
         */

        else
            class =
                (beingstalked || turns - putonseeinv < 20)
                    ? 1999
                    : ((havefood(0) && Level > 15 && Level < 26) ? 1000 : 0) +
                          300;
    }
    ...
    /* A ring of slow digestion */
        ...
        if (havefood(0) && !itemis(i, INUSE) &&
            ...
            class = havefood(3)
                        ...
                        : havefood(2)
                              ? 1300
                              : havefood(1) ? 1500 : havefood(0) ? 1900 : 1999;
        }
    }
...
int havebow(int k, int print) {
    ...
    if (print) {
        ...
        for (i = 0; i < n; i++)
            mvprintw(i + 3, 8, "%2d: %5d %s", i + 1, bowval[i],
                     itemstr(bowind[i]));
    }
...
int bowclass(int i) {
    ...
    if (inven[i].type == thrower && stlmatch(inven[i].str, "short bow") &&
...
int havemissile() {
    ...
    if (obj < 0) { /* Not wielding bow or no arrows */
        for (i = 0; i < invcount; ++i)
            if (inven[i].count > 0 && inven[i].count < fewest &&
                !itemis(i, INUSE) &&
                (inven[i].type == missile || stlmatch(inven[i].str, "spear") ||
...
int havearrow() {
    ...
    for (arr = 0; arr < invcount; arr++)
        if (inven[arr].type == missile && inven[arr].count == 1 &&
