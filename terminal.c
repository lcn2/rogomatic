...
cleanup_termattr(bool unlink_saved_termattr)
{
	...
	if (cl_ret < 0) {
	    fprintf(stderr, "Warning: close of: %s failed: %s\n", (termattr_path == NULL ? "saved_termattr" : termattr_path),
		    strerror(errno));
	}
    }
...
save_termattr(char *dir)
{
     ...
     */
    termattr_fd = open(termattr_path, O_WRONLY | O_TRUNC | O_APPEND | O_CREAT | O_CLOEXEC,
		       S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH); /* mode 0644 */
    if (termattr_fd < 0) {
	...
	/* report failed to save terminal state */
	fprintf(stderr, "Warning: terminal attributes not saved: write size of stdin state: %zd != %zu", wr_ret,
		sizeof(tattr_stdin));
	cleanup_termattr(true);
	return false;
    }
	...
	/* report failed to save terminal state */
	fprintf(stderr, "Warning: terminal attributes not saved: write size of stdout state: %zd != %zu", wr_ret,
		sizeof(tattr_stdout));
	cleanup_termattr(true);
	return false;
    }
	...
	/* report failed to save terminal state */
	fprintf(stderr, "Warning: terminal attributes not saved: write size of stderr state: %zd != %zu", wr_ret,
		sizeof(tattr_stderr));
	cleanup_termattr(true);
	return false;
    }
...
restore_termattr(char *dir)
{
	...
	fprintf(stderr, "Warning: terminal attributes not restored: file: %s size: %lld != %zu\n", termattr_path,
		(long long)stat_buf.st_size, sizeof(tattr_stdin) + sizeof(tattr_stdout) + sizeof(tattr_stderr));
	cleanup_termattr(false);
	return false;
    }
