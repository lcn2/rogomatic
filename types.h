...
#  define ordinal(n)                                                                                                    \
      ((((n) / 10) % 10 == 1) ? "th" : ((n) % 10 == 1) ? "st" : ((n) % 10 == 2) ? "nd" : ((n) % 10 == 3) ? "rd" : "th")
#  define ctrl(c) ((c) & 037)
#  define ESC ctrl('[')
 ...
 */
#  define on(attribute)                                                                                                  \
      (valrc((atrow), (atcol)) ? ((attribute) & scrmap[(atrow)][(atcol)])                                                \
...
#  define see(character)                                                                                                  \
      (valrc((atrow), (atcol)) ? ((character) == screen[(atrow)][(atcol)])                                                \
...
#  define setrc(attribute, r, c)                                                                                               \
      (valrc((r), (c)) ? (scrmap[r][c] |= (attribute))                                                                         \
...
#  define set(attribute)                                                                                                  \
      (valrc((atrow), (atcol)) ? (scrmap[atrow][atcol] |= (attribute))                                                    \
...
typedef enum { strange, food, potion, Scroll, wand, ring, hitter, thrower, missile, armor, amulet, gold, none } stuff;
...
typedef struct {
    int expdam, maxdam, mtokill;
} attrec;

typedef struct {
    int gamecnt, gamesum, inittime, timeswritten;
} lrnrec;
...
extern int sleepvalue(int r, int c, int depth __attribute__((__unused__)), int *val, int *avd,
		      int *cont __attribute__((__unused__)));
extern int setpsd(int print);
extern int downvalue(int r, int c, int depth __attribute__((__unused__)), int *val, int *avd,
		     int *cont __attribute__((__unused__)));
