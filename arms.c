...
havearmor(int k, int print, int rustproof)
{
	...
	if (inven[i].count && inven[i].type == armor && !(rustproof && willrust(i))) {
	    n++;
	    w = armorclass(i);
...
haveweapon(int k, int print)
{
	...
	if (weapval[j] == weapval[j - 1]) {
	    ...
	    if (!itemis(w, ENCHANTED) && itemis(i, ENCHANTED) && !itemis(w, KNOWN) && !itemis(i, KNOWN)) {
		swap(weapind[j], weapind[j - 1]);
	    }
	}
    }
...
weaponclass(int i)
{
    ...
    } else if (creative && inven[i].type == missile && stlmatch(inven[i].str, "arrow")) {
	;
	/* Not a valid weapon */
    } else {
	return (0);
    ...
    } else if (creative && version <= RV36B && usingarrow && goodarrow > 20 && i == currentweapon) {
	return (1800);

    } else if (creative && version <= RV36B && stlmatch(inven[i].str, "arrow") && inven[i].count == 1 && !itemis(i, WORTHLESS) &&
...
ringclass(int i)
{
	...
	if (magicplus > 0) {
	    class = (havefood(1) ? 1000 : 0) + 450;
	}

	/* A ring of add strength */
    } else if (stlmatch(inven[i].str, "add strength")) {
	if (itemis(i, INUSE) && magicplus > 0) {
	    if (hitbonus(Str) == hitbonus(Str - magicplus * 100) && damagebonus(Str) == damagebonus(Str - magicplus * 100)) {
		class = 400;
	    } else {
	...
	if (!itemis(i, INUSE) && wearing("sustain strength") != NONE) {
	    ...
	    class = (havefood(3) ? 1000 : 0) + (Level > 12 ? 150 : Str > 2000 ? 700 : Str > 1600 ? Str - 1200 : 100);
	}
	...
	if (!itemis(i, INUSE) && wearing("see invisible") != NONE) {
	    ...
	    class =
		(beingstalked || turns - putonseeinv < 20) ? 1999 : ((havefood(0) && Level > 15 && Level < 26) ? 1000 : 0) + 300;
	}
	...
	if (havefood(0) && !itemis(i, INUSE) && wearing("slow digestion") != NONE) {
	    class = 1001;

	} else {
	    class = havefood(3) ? 1100 : havefood(2) ? 1300 : havefood(1) ? 1500 : havefood(0) ? 1900 : 1999;
	}

	/* A ring of teleportation */
    } else if (stlmatch(inven[i].str, "telportation") || stlmatch(inven[i].str, "teleportation")) {
	class = 0;
	...
	if (!itemis(i, INUSE) && wearing("stealth") != NONE) {
	    ...
	    class = (havefood(1) ? 1000 : 0) + (Level > 17 ? 850 : Level > 12 ? 700 : 300);
	}
	...
	if (Level < 9 || cursedarmor) {
	    ...
	    class = (havefood(1) ? 1000 : 0) + 200 * armdiff;
	}
...
bowclass(int i)
{
    ...
    if (inven[i].type == thrower && stlmatch(inven[i].str, "short bow") && havemult(missile, "arrow", 5) != NONE) {
	class = 35;
    } else {
	return (0);
    }
...
havemissile(void)
{
	...
	for (i = 0; i < invcount; ++i) {
	    if (inven[i].count > 0 && inven[i].count < fewest && inven[i].type == missile && stlmatch(inven[i].str, "arrow")) {
		obj = i;
	...
	for (i = 0; i < invcount; ++i) {
	    if (inven[i].count > 0 && inven[i].count < fewest && !itemis(i, INUSE) &&
		(inven[i].type == missile || stlmatch(inven[i].str, "spear") || stlmatch(inven[i].str, "dagger") ||
...
havearrow(void)
{
	...
	if (inven[arr].type == missile && inven[arr].count == 1 && stlmatch(inven[arr].str, "arrow")) {
	    return (arr);
	}
    }
...
damagebonus(int strength)
{
	...
	if (strength < 1600) {
	    ...
	    bonus = 1;
	} else if (strength < 1801) {
	    bonus = 2;
	} else if (strength < 1876) {
	    bonus = 3;
	} else if (strength < 1891) {
	    bonus = 4;
	} else if (strength < 1900) {
	    bonus = 5;
	} else {
...
setbonuses(void)
{
    ...
    if (leftring != NONE && stlmatch(inven[leftring].str, "dexterity") && inven[leftring].phit != UNKNOWN) {
	gplushit += inven[leftring].phit;
    }

    if (rightring != NONE && stlmatch(inven[rightring].str, "dexterity") && inven[rightring].phit != UNKNOWN) {
    ...
    if (leftring != NONE && stlmatch(inven[leftring].str, "add damage") && inven[leftring].pdam != UNKNOWN) {
	gplusdam += inven[leftring].pdam;
    }

    if (rightring != NONE && stlmatch(inven[rightring].str, "add damage") && inven[rightring].pdam != UNKNOWN) {
