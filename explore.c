...
#define SEARCHES(r, c)                                                                                                \
    (if_onrc(DEADEND, r, c)                                                                                           \
	 ? ((version < RV53A || !isexplored(r, c)) ? (timestosearch + k_door / 5) : (timestosearch - k_door / 5 + 5)) \
	 : timestosearch)
...
static int rogo_connect[RGRID][4] = {/* Room  top    bot   left  right*/
				     /* 0 */ {-1, 3, -1, 1},
				     /* 1 */ {-1, 4, 0, 2},
				     /* 2 */ {-1, 5, 1, -1},
				     /* 3 */ {0, 6, -1, 4},
...
static int gotovalue(int r, int c, int depth __attribute__((__unused__)), int *val, int *avd,
		     int *cont __attribute__((__unused__)));
static int wallkind(int r, int c);
static int expinit(void);
static int roominit(void);
static int expvalue(int r, int c, int depth __attribute__((__unused__)), int *val, int *avd, int *cont);
...
#define N 100
static int secretvalues[16] = {0, N - 24, N - 22, N - 20, N - 9, N - 6, N - 5, N - 5, N - 3, N - 2, N - 1, N - 1, N, N, N, N};

static int secretcont[16] = {0, 16, 15, 14, 4, 2, 2, 2, 1, 0, 0, 0, 0, 0, 0, 0};
...
static int
gotovalue(int r, int c, int depth __attribute__((__unused__)), int *val, int *avd, int *cont __attribute__((__unused__)))
{
    /* firewall */
...
int
sleepvalue(int r, int c, int depth __attribute__((__unused__)), int *val, int *avd, int *cont __attribute__((__unused__)))
{
    /* firewall */
    if (val == NULL || avd == NULL) {
	quit(1, "ERROR: %s: file: %s line: %d dungeon: %u NULL arg(s): val: %p avd: %p\n", __func__, __FILE__, __LINE__, dnum,
...
wallkind(int r, int c)
{
    ...
    case '-':
	if (if_onrc(ROOM, r + 1, c)) {
	    return (TOPW);
	} else if (if_onrc(ROOM, r - 1, c)) {
	    return (BOTW);
	} else {
	    return (CORNERW);
	}
...
setpsd(int print)
{
	...
	for (j = 1; j < C - 1; j++) {
	    ...
	    if (attempt > 3 && !if_onrc(BEEN | DOOR | HALL | ROOM | WALL | STAIRS, i, j) && nextto(CANGO, i, j)) {
		if (!if_onrc(PSD, i, j)) {
		    numberpsd++;
		}
		setrc(PSD, i, j);
	    ...
	    } else if (attempt > 0 && !if_onrc(BEEN | DOOR | HALL | ROOM | WALL | STAIRS, i, j) && mazedoor(i, j)) {
		if (!if_onrc(PSD, i, j)) {
		    numberpsd++;
		}
		setrc(PSD, i, j);
	    ...
	    } else if (version >= RV53A && !if_onrc(BEEN | DOOR | HALL | ROOM | WALL | STAIRS, i, j) && nextto(DOOR, i, j)) {
		if (!if_onrc(PSD, i, j)) {
		    numberpsd++;
		}
		setrc(PSD, i, j);
		...
		/* Set Possible Secret Door for dead end corridors */
		       ...
		       (if_onrc(HALL, i - 1, j) + if_onrc(HALL, i + 1, j) + if_onrc(HALL, i, j - 1) + if_onrc(HALL, i, j + 1) ==
			HALL) &&
		       !(if_onrc(HALL | DOOR, i - 1, j - 1) || if_onrc(HALL | DOOR, i + 1, j + 1) ||
			 if_onrc(HALL | DOOR, i - 1, j + 1) || if_onrc(HALL | DOOR, i + 1, j - 1)) &&
		       canbedoor(i, j)) {
		if (!if_onrc(PSD, i, j)) {
		    numberpsd++;
		...
		if ((k = wallkind(i, j)) >= 0) { /* A legit sort of wall */
			...
			if (whereto >= 0 && whereto < RGRID && (attempt > 1 || room[whereto] == 0)) {
			    if (!if_onrc(PSD, i, j)) {
				numberpsd++;
			    }
...
downvalue(int r, int c, int depth __attribute__((__unused__)), int *val, int *avd, int *cont __attribute__((__unused__)))
{
	...
	quit(1, "ERROR: %s: file: %s line: %d dungeon: %u NULL arg(s): val: %p avd: %p cont: %p\n", __func__, __FILE__, __LINE__,
	     dnum, (void *)val, (void *)avd, (void *)cont);
	not_reached();
    }
    if (!valrc(r, c)) {
	return 0;
...
expvalue(int r, int c, int depth __attribute__((__unused__)), int *val, int *avd, int *cont)
{
	...
	*cont = 4; /* Look for something better */
	...
	mvaddch(r, c, 'o');
	dwait(D_SCREEN, __func__, "Value: %d", v);
    }
...
secretvalue(int r, int c, int depth __attribute__((__unused__)), int *val, int *avd, int *cont)
{
    ...
    *val = 0;
    ...
    if (v > 0) {
	if (version >= RV53A && onrc(DOOR | BEEN, r, c) == (DOOR | BEEN) &&
	    (if_onrc(CANGO | WALL, r + 1, c) == 0 || if_onrc(CANGO | WALL, r - 1, c) == 0 ||
	     if_onrc(CANGO | WALL, r, c + 1) == 0 || if_onrc(CANGO | WALL, r, c - 1) == 0)) {
	    *val = v + 100;
	    *cont = 5;
	} else {
	    v = min(15, v);
...
secret(void)
{
	...
	(if_seerc(' ', atrow + 1, atcol) || if_seerc(' ', atrow - 1, atcol) || if_seerc(' ', atrow, atcol + 1) ||
	 if_seerc(' ', atrow, atcol - 1)) &&
	SEARCHES(atrow, atcol) < timestosearch + 20) {
	int count = timessearched[atrow][atcol] + 1;
	saynow("Searching dead end door (%d,%d) for the %d%s time...", atrow, atcol, count, ordinal(count));
	command(T_DOORSRCH, "s");
	return (1);
    }

    /* Verify that we are actually at a dead end */
    if ((if_onrc(CANGO, atrow - 1, atcol) + if_onrc(CANGO, atrow, atcol - 1) + if_onrc(CANGO, atrow + 1, atcol) +
	 if_onrc(CANGO, atrow, atcol + 1)) != CANGO) {
	return (0);
    }

    /* If Level 1 or edge of screen: dead end cannot be room, mark and return */
    if ((Level == 1 && attempt == 0) || (version < RV53A && (atrow <= 1 || atrow >= R - 2 || atcol <= 0 || atcol >= C - 1))) {
    ...
    if (nexttowall(atrow, atcol) || (canbedoor(atrow, atcol) && (version >= RV53A || !isexplored(atrow, atcol)))) {
	setrc(DEADEND, atrow, atcol);

	if ((SEARCHES(atrow, atcol) - timessearched[atrow][atcol]) > 0) {
	    int count = timessearched[atrow][atcol] + 1;
	    saynow("Searching dead end (%d,%d) for the %d%s time...", atrow, atcol, count, ordinal(count));
	    command(T_DOORSRCH, "s");
	    return (1);
	} else {
	    markexplored(atrow, atcol);
...
doorexplore(void)
{
	...
	searchcount++;
	saynow("Searching square (%d,%d) for the %d%s time...", atrow, atcol, searchcount, ordinal(searchcount));
	command(T_DOORSRCH, "s");
	return (1);
    }
...
safevalue(int r, int c, int depth __attribute__((__unused__)), int *val, int *avd, int *cont)
{
	...
	*avd += 500;
	...
	for (k = 0; k < DNUM; k++) {
	    if (if_onrc(CANGO, r + deltr[k], c + deltc[k]) && if_onrc(CANGO, r + deltr[k], c) && if_onrc(CANGO, r, c + deltc[k])) {
		v++;
	    }
	}
...
avoid(void)
{
    if (creative && !foundarrowtrap && !usingarrow) {
	return (0);
    } else if (Level < 5) {
	return (0); /* Don't bother */
    } else {
	return (2); /* Avoid a little */
    }
}
...
archmonster(int m, int trns)
{
    ...
    if (mlist[m].q != ASLEEP) {
	dwait(D_BATTLE, __func__, "monster not asleep");
	return (0);
    }
	...
	dwait(D_BATTLE, __func__, "made a move");
	return (1);
    }
    ...
    mlist[m].q = AWAKE;
    dwait(D_BATTLE, __func__, "waking him up");
	...
	agoalr = mr;
	agoalc = mc; /* Go here to pick up what (s)he drops */
    }
...
archeryinit(void)
{
	...
	for (dist = 1, r = archrow + dr, c = archcol + dc; if_onrc(CANGO | HALL | MONSTER, r, c) == CANGO;
	     r += dr, c += dc, dist++) {
...
movetorest(void)
{
	...
	dwait(D_SEARCH, __func__, "already on square");
	return (0);
    }
...
restvalue(int r, int c, int depth __attribute__((__unused__)), int *val, int *avd, int *cont)
{
	...
	*avd = 5;
    ...
    if (if_onrc(TRAPDOR | TELTRAP, r - 1, c - 1) || if_onrc(TRAPDOR | TELTRAP, r + 1, c - 1) ||
	if_onrc(TRAPDOR | TELTRAP, r - 1, c + 1) || if_onrc(TRAPDOR | TELTRAP, r + 1, c + 1)) {
	*val += 80;
	*cont = 99;
    }

    if (if_onrc(TRAPDOR | TELTRAP, r - 1, c) || if_onrc(TRAPDOR | TELTRAP, r + 1, c) || if_onrc(TRAPDOR | TELTRAP, r, c - 1) ||
	...
	*cont = 99;
	...
	for (dir = 0; dir < DNUM; dir += 2) {
	    ...
	    for (count = 0, ar = r + dr, ac = c + dc; if_onrc(CANGO | HALL | MONSTER, ar, ac) == CANGO;
		 ar += dr, ac += dc, count++) {
		/* Bonus of 'count' if this square covers a door */
		if (if_onrc(DOOR, ar + deltr[(dir + 2) % DNUM], ac + deltc[(dir + 2) % DNUM])) {
		    *val += count;
		}

		if (if_onrc(DOOR, ar + deltr[(dir + 6) % DNUM], ac + deltc[(dir + 6) % DNUM])) {
		    *val += count;
		}

		if (if_onrc(DOOR, ar, ac)) {
		    *val += count;
		}
	    }
	}
	...
	if (if_onrc(DOOR, r - 1, c - 1) && (rm != 0 && rm != 1 && rm != 3)) {
	    *val += 80;
	    *cont = 99;
	}

	if (if_onrc(DOOR, r + 1, c - 1) && (rm != 3 && rm != 6 && rm != 7)) {
	    *val += 80;
	    *cont = 99;
	}

	if (if_onrc(DOOR, r - 1, c + 1) && (rm != 1 && rm != 2 && rm != 5)) {
	    *val += 80;
	    *cont = 99;
	}

	if (if_onrc(DOOR, r + 1, c + 1) && (rm != 5 && rm != 7 && rm != 8)) {
	    *val += 80;
	    *cont = 99;
	}

	/* Bonus for door also orthogonally away */
	if (if_onrc(DOOR, r, c - 1) || if_onrc(DOOR, r - 1, c) || if_onrc(DOOR, r, c + 1) || if_onrc(DOOR, r + 1, c)) {
	    *val += 30;
	    *cont = 99;
	}
    }
